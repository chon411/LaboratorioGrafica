...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::scale(model, glm::vec3(2.0f, 0.2f, 2.5f));

		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);

		// PATAS
		float alturaPata = -1.5f;
		float separacionX = 2.0f;
		float separacionZ = 1.0f;

		// Pata 1
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(3.0f - separacionX, alturaPata, separacionZ));
		...
		model = glm::scale(model, glm::vec3(3.0f, 0.1f, 2.5f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		glBindVertexArray(0);
